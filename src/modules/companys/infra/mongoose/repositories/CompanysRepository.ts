...
export class CompanysRepository implements ICompanysRepository {
	...
	async create({ idUsers, name, address, phone }: ICreateCompanysDTO): Promise<ICompanyModel> {
		try {
	...
	async updateById({ id, idUsers, name, address, phone }: ICreateCompanysDTO): Promise<void> {
		try {
