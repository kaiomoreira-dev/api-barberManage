...
exports.ListServiceExecutedByServiceIdUseCase = void 0;
...
let ListServiceExecutedByServiceIdUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute(idServices) {
    if (!(0, _ensureId.ensureId)(idServices)) {
      throw new _AppError.AppError("ServiceExecuted not found", 404);
    }
    const checkServiceExist = await this.servicesRepository.findById(idServices);
    if (!checkServiceExist) {
      throw new _AppError.AppError("Service not found", 404);
    }
    const servicesExecuted = await this.serviceExecutedRepository.listByServiceId(checkServiceExist.id);
...
exports.ListServiceExecutedByServiceIdUseCase = ListServiceExecutedByServiceIdUseCase;